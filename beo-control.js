class BeoControl extends HTMLElement {
  constructor() {
    super();
    ...
    if (this._state) {
      root.getElementById('content').innerHTML = `
        <div class='flex justify'>
          ...
          ${this._state !== 'off' && this._state !== 'unavailable' ? this._renderBeoControls() : '' }
          <div>
      ...
      if (this._state !== 'unavailable') {
        ...
        if (this._state !== 'off') this._setupBeoControls();
      };
    }
  }
  ...
  _renderBeoControls() {
    return `
      <div class='flex'>
        <paper-icon-button class='volume_down' icon='${this._icons["volume_down"]}'></paper-icon-button>
        <paper-icon-button class='volume_up' icon='${this._icons["volume_up"]}'></paper-icon-button>
      </div>`;
  }
  ...
  _setupBeoControls() {
    const root = this.shadowRoot;
    let volume_up = root.querySelector('.volume_up');
    let volume_down = root.querySelector('.volume_down');

    volume_down.addEventListener('click', e => this._callService(e, 'volume_down'));
    volume_up.addEventListener('click', e => this._callService(e, 'volume_up'));
  }
...
customElements.define('beo-control', BeoControl);
